...
import fastcampus.part2.chapter7.forecast.ForecastData
...
object WeatherRepository {
    ...
    fun getVillageForecast(
        longitude: Double,
        latitude: Double,
        successCallback: (List<ForecastData>) -> Unit,
        ...
        service.getVillageForecast(
            ...
            override fun onResponse(
                ...
                val forecastDateTimeMap = mutableMapOf<String, ForecastData>()
                ...
                for (forecast in forecastList) {
                    ...
                    forecastDateTimeMap["${forecast.forecastDate}/${forecast.forecastTime}"]?.apply {
                        when (forecast.category) {
                            WeatherCategory.POP -> precipitation = forecast.forecastValue.toInt()
                            WeatherCategory.PTY -> precipitationType = transformRainType(forecast)
                            WeatherCategory.SKY -> sky = transformSky(forecast)
                            WeatherCategory.TMP -> temperature = forecast.forecastValue.toDouble()
                            else -> {}
                        }
                    }
                }
